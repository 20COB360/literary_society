...
if ($_SERVER['REQUEST_METHOD']=='POST') {
  if(isset($_POST['update'])){
    ...
    $sql="UPDATE `poems` SET `author` = '$name', `title` = '$title', `desc` = '$desc', `poem` = '$content',`featured`='$featured',`author_bio`='$author_bio'  WHERE `poem_id`='$sno'";
...
if (($_SERVER['REQUEST_METHOD']=='GET') ){
  ...
  if (isset($_GET['delete'])) {
            ...
            $sql="select * from poems where poems.poem_id=$sno";
            $result=mysqli_query($conn,$sql);
    ...
    $sql="DELETE FROM `poems` WHERE `poems`.`poem_id` = $sno";
...
if($insert){
  ...
  <strong>Success! </strong>Poetry hasbeen inserted successfully.
...
if($update){
  echo '<div class="alert alert-success alert-dismissible fade show" role="alert">
  <strong>Success! </strong>Poetry hasbeen updated successfully.
  <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
</div>';
}
if($delete){
  echo '<div class="alert alert-success alert-dismissible fade show" role="alert">
  <strong>Success! </strong>Poetry hasbeen deleted successfully.
  <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
</div>';
}
...
?>
    ...
    <div class="container my-5">
        <h2>CONTROL PANEL - ALL PUBLISHED POETRY </h2>
...
<td> <a id=".$row['poem_id']." href='./poetry_images.php?key=".$row['poem_id']."' style='color:white;text-decoration:none; '><button  class='btn btn-warning my-1 ' >Edit Images</button></a> <button  class='btn btn-success my-1 edit' data-bs-toggle='modal' data-bs-target='#editModal' id=".$row['poem_id'].">Edit</button>  <button  class='btn btn-danger deletes' id=". 99999999 . $row['poem_id']." >Delete</button></td>
   ...
   // Deleting Element
    ...
    arr2.forEach((element) => {
      element.addEventListener("click",(e)=>{
        ...
        confirmation= confirm("Do you want to delete this poetry ?");
        if(confirmation){
          // console.log("yes")
          window.location=`./poetry.php?delete=${sno}`;
        }
