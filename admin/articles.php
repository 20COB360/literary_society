...
if ($_SERVER['REQUEST_METHOD']=='POST') {
  if(isset($_POST['update'])){
    ...
    $sql="UPDATE `articles` SET `author` = '$name', `title` = '$title', `desc` = '$desc', `article` = '$content',`featured`='$featured',`author_bio`='$author_bio' WHERE `article_id`='$sno'";
...
if (($_SERVER['REQUEST_METHOD']=='GET') ){
  ...
  if (isset($_GET['delete'])) {
    ...
    $sql="select * from articles where articles.article_id=$sno";
    $result=mysqli_query($conn,$sql);
   ...
   if($author_image!=""){
    ...
    $sql="DELETE FROM `articles` WHERE `articles`.`article_id` = $sno";
...
if($insert){
  ...
  <strong>Success! </strong>Article hasbeen inserted successfully.
...
if($update){
  echo '<div class="alert alert-success alert-dismissible fade show" role="alert">
  <strong>Success! </strong>Article hasbeen updated successfully.
  <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
</div>';
}
if($delete){
  echo '<div class="alert alert-success alert-dismissible fade show" role="alert">
  <strong>Success! </strong>Article hasbeen deleted successfully.
  <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
</div>';
}
...
?>
    ...
    <div class="container my-5">
        <h2>CONTROL PANEL - ALL ARTICLES PUBLISHED </h2>
...
<td>". $row['article']."</td>
<td>". $row['featured']."</td>
<td><a id=".$row['article_id']." href='./article_images.php?key=".$row['article_id']."' style='color:white;text-decoration:none; '><button  class='btn btn-warning my-1 ' >Edit Images</button></a>  <button  class='btn btn-success my-1 edit' data-bs-toggle='modal' data-bs-target='#editModal' id=".$row['article_id'].">Edit</button>  <button  class='btn btn-danger deletes' id=". 99999999 . $row['article_id']." >Delete</button></td>
   ...
   // Deleting Element
    ...
    arr2.forEach((element) => {
      element.addEventListener("click",(e)=>{
        ...
        confirmation= confirm("Do you want to delete this article ?");
        if(confirmation){
          // console.log("yes")
          window.location=`./articles.php?delete=${sno}`;
        }
